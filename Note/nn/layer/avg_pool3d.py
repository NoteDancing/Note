...
class avg_pool3d:
    ...
    def __call__(self, data):
        if not isinstance(self.padding, str):
            padded_data = self.zeropadding3d(data)
            padding = 'VALID'
        else:
            padded_data = data
        ...
        if not self.count_include_pad and not isinstance(self.padding, str):
            ...
            valid_mask = tf.ones_like(data, dtype=tf.float32)
            valid_mask = self.zeropadding3d(valid_mask)

            # Apply the same pooling operation to the mask
            valid_counts = tf.nn.avg_pool3d(
            ...
            pooled = pooled * (k_d * k_h * k_w) / valid_counts
